...
users = {
    ...
    "Steven": bcrypt.hashpw("Gerrard".encode('utf-8'), bcrypt.gensalt())
}
...
def login():
    if request.method == 'POST':
        ...
        if user and user.verify_password(password):
            login_user(user)
            logger.info(f'Utilisateur {username} a réussi la connexion.')
            return redirect(url_for('home'))
        else:
            flash('Invalid credentials.')
            logger.info(f'La tentative de connexion a échoué pour l\'utilisateur {username}.')
    return render_template('login.html')

# Chemins relatifs par rapport au dossier src
chemin_fichier_csv = os.path.join("..", "data/api", "premiere_moitie_2021-22.csv")
chemin_fichier_excel = os.path.join("..", "data/api", "seconde_moitie_2021-22.xlsx")
...
def get_matches():
    ...
    try:
        logger.info(f"Recherche des matchs pour la journée {jour}")
...
@login_required
def predict():
    logger.info(f'Utilisateur {current_user.id} a demandé une prédiction.')
    ...
    try:
        ...
        logger.info(f"Équipes reçues pour prédiction: {home_team} vs {away_team}")

        # Encodage des variables catégorielles 'HomeTeam' et 'AwayTeam'
        # logger.info("Encodage des variables 'HomeTeam' et 'AwayTeam'...")
        # Importer le mapping
        equivalence_equipes = {
            'Arsenal': 0,
            'Aston Villa': 1,
        ...
        # logger.info(home_team_code)
        # logger.info(away_team_code)
        ...
        # logger.info(donnees['HomeTeam'])
        # logger.info(donnees['AwayTeam'])
        ...
        logger.info(f'Utilisateur {current_user.id} a effectué une prédiction. Résultat : {result}')
